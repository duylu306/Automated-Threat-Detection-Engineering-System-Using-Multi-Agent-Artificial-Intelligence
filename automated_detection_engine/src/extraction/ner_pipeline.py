...
class SecurityNERPipeline:
    ...
    def _extract_gliner(self, text: str) -> list[SecurityEntity]:
        """Zero-shot extraction với GLiNER có Chunking để tránh bị truncate."""
        self._load_gliner()
        if not self._gliner:
            return []

        gliner_labels = [
            "malware family", "threat actor group", "attack campaign",
            "hacking tool", "vulnerability", "targeted organization",
            "command and control server"
        ]

        entities = []
        try:
            # Băm nhỏ văn bản: Cắt mỗi đoạn 1000 ký tự để không vượt quá 384 tokens
            chunk_size = 1000
            for i in range(0, len(text), chunk_size):
                ...
                label_map = {
                    "malware family": "MALWARE", "threat actor group": "THREAT_ACTOR",
                    "attack campaign": "CAMPAIGN", "hacking tool": "TOOL",
                    "vulnerability": "VULNERABILITY", "targeted organization": "TARGET",
                ...
                for ent in raw_entities:
                    ...
                    entities.append(SecurityEntity(
                        text=ent["text"],
                        label=std_label,
                        start=ent["start"] + i, # Cộng dồn offset của chunk
                        end=ent["end"] + i,
                        confidence=ent["score"],
                        source="gliner",
                    ))
